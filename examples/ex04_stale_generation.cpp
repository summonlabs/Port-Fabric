// Example 4: stale generation rejection versus exact idempotent replay.
#include <cstdio>
#include <string>
...
int main() {
  ...
  portfabric::PortFabricEngine engine(config_for("example-04", ""), &fixture.provider,
                                      &fixture.adapter);
  ...
  configure.configuration = base_configuration(port, fixture.device, *capability, portfabric::SyntheticPortClass::Server10G);
  if (report("configure", engine.configure(configure)) != 0) {
    return 1;
  }
  const portfabric::PortConfigurationGeneration first = engine.configuration(port)->generation;

  // Exact replay: same attempt identity and same payload.
  if (report("exact_replay", engine.configure(configure)) != 0) {
    return 1;
  }
  std::printf("generation_after_replay=%s\n", engine.configuration(port)->generation.to_string().c_str());

  // Reconfigure, then replay the older expectation.
  portfabric::ReconfigureRequest reconfigure;
  reconfigure.envelope = envelope_for(port, "agent-a", "boot-a", "reconfigure-1", engine.epoch());
  reconfigure.envelope.authority.ownership = ownership->id;
  reconfigure.envelope.authority.expected_configuration = first;
  reconfigure.configuration = *engine.configuration(port);
  reconfigure.configuration.mtu = *portfabric::Mtu::create(9000);
  if (report("reconfigure", engine.reconfigure(reconfigure)) != 0) {
    return 1;
  }

  configure.envelope.authority.expected_configuration = first;
  configure.envelope.authority.attempt = portfabric::MutationAttemptId::from_validated("stale-1");
  const portfabric::MutationResult stale = engine.configure(configure);
  std::printf("stale_configuration_expectation %s\n", stale.outcome.to_string().c_str());

  portfabric::AdministrativeRequest administrative;
  administrative.envelope = envelope_for(port, "agent-a", "boot-a", "admin-stale", engine.epoch());
  administrative.envelope.authority.ownership = ownership->id;
  administrative.envelope.authority.expected_configuration = first;
  administrative.op = portfabric::AdministrativeOp::Disable;
  std::printf("stale_administrative       %s\n",
              engine.administrative(administrative).outcome.to_string().c_str());
  std::printf("explanation=%s\n", stale.explanation.to_string().c_str());
  return 0;
}
