// Example 10: breakout. One physical port becomes several derived logical ports.
// Every child carries explicit derivation provenance and an independent
// configuration generation. This example is SYNTHETIC: no breakout hardware is
...
int main() {
  ...
  portfabric::PortFabricEngine engine(config_for("coordinator", "", 1), &fixture.provider,
                                      &fixture.adapter);
  const portfabric::PortId parent = fixture.port(1);
  if (!prepare_port(engine, fixture, parent, "agent-a", "boot-a",
                    portfabric::DeviceGeneration::from_value(1), 1)) {
    return 1;
  }
  std::printf("parent_lifecycle=%s\n",
              std::string(portfabric::to_string(*engine.lifecycle(parent))).c_str());

  const std::vector<portfabric::PortId> children =
      fixture.fabric.add_breakout_children(parent, 4);
  std::printf("children=%zu\n", children.size());

  portfabric::OwnershipRequest claim;
  claim.envelope = envelope_for(parent, "agent-a", "boot-a", "claim-children", engine.epoch());
  claim.owner_kind = portfabric::PortOwnerKind::SwitchAgent;
  claim.owner = portfabric::OwnerId::from_validated("owner-agent-a");
  (void)engine.claim_ownership(claim);
  const auto ownership = engine.ownership(parent);

  for (std::size_t index = 0; index < children.size(); ++index) {
    const portfabric::PortId& child = children[index];
    portfabric::PortBindingRequest binding;
    binding.port = child;
    binding.parent_device = fixture.device;
    binding.device_generation = portfabric::DeviceGeneration::from_value(1);
    binding.epoch = engine.epoch();
    binding.attempt = portfabric::MutationAttemptId::from_validated(
        "bind-child-" + std::to_string(index));
    (void)engine.bind_port(binding);
    portfabric::OwnershipRequest child_claim = claim;
    child_claim.envelope.port = child;
    child_claim.envelope.authority.attempt = portfabric::MutationAttemptId::from_validated(
        "claim-child-" + std::to_string(index));
    (void)engine.claim_ownership(child_claim);
    const auto child_ownership = engine.ownership(child);
    portfabric::ConfigureRequest configure;
    configure.envelope.port = child;
    configure.envelope.authority.epoch = engine.epoch();
    configure.envelope.authority.publisher = portfabric::PublisherId::from_validated("agent-a");
    configure.envelope.authority.boot = portfabric::WorkerBootId::from_validated("boot-a");
    configure.envelope.authority.ownership = child_ownership->id;
    configure.envelope.authority.attempt = portfabric::MutationAttemptId::from_validated(
        "configure-child-" + std::to_string(index));
    configure.configuration = base_configuration(child, fixture.device,
                                                 *engine.capability_binding(child),
                                                 portfabric::SyntheticPortClass::Spine400G);
    configure.configuration.mode = portfabric::PortMode::Logical;
    configure.configuration.logical_identity =
        portfabric::LogicalPortId::from_validated("lgl-" + child.value());
    configure.configuration.derived_from = parent;
    configure.configuration.child_index = static_cast<std::uint32_t>(index + 1);
    // A physical port split four ways: four lanes, each child realising an equal
    // share of the parent rate, so the lane count divides across the fan-out.
    configure.configuration.breakout = portfabric::BreakoutMode::X4;
    configure.configuration.lanes =
        *portfabric::LaneConfiguration::for_total(4, *portfabric::PortSpeed::parse("200G"));
    configure.configuration.speed.rate = *portfabric::PortSpeed::parse("200G");
    configure.configuration.fec = portfabric::FecMode::ReedSolomon544;
    const portfabric::MutationResult result = engine.configure(configure);
    std::printf("child[%zu]=%s result=%s\n", index, child.to_string().c_str(),
                result.outcome.to_string().c_str());
  }
  (void)ownership;
  std::printf("derived_ports=%zu\n", engine.derived_ports(parent).size());
  std::printf("snapshot=%s\n", engine.snapshot().to_string().c_str());
  return 0;
}
