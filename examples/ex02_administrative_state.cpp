// Example 2: administrative state is owned by Port Fabric and is never derived
// from operational link condition.
#include <cstdio>
#include <string>
...
int main() {
  ...
  portfabric::PortFabricEngine engine(config_for("example-02", ""), &fixture.provider,
                                      &fixture.adapter);
  ...
  const Step steps[] = {
      {portfabric::AdministrativeOp::Disable, "disable"},
      {portfabric::AdministrativeOp::Enable, "enable"},
      {portfabric::AdministrativeOp::Drain, "drain"},
      {portfabric::AdministrativeOp::EnterMaintenance, "maintenance"},
      {portfabric::AdministrativeOp::Resume, "resume"},
  };
  int index = 0;
  for (const Step& step : steps) {
    portfabric::AdministrativeRequest request;
    request.envelope = envelope_for(port, "agent-a", "boot-a",
                                    std::string("admin-") + step.label, engine.epoch());
    ...
    request.op = step.op;
    if (report(step.label, engine.administrative(request)) != 0) {
      return 1;
    }
    std::printf("  lifecycle=%s administrative=%s\n",
                std::string(portfabric::to_string(*engine.lifecycle(port))).c_str(),
                std::string(portfabric::to_string(*engine.administrative_state(port))).c_str());
    ++index;
  }
  std::printf("steps=%d\n", index);
  return 0;
