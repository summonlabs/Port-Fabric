// Example 1: bind a canonical port identity, claim ownership and commit the
...
int main() {
  ...
  portfabric::PortFabricEngine engine(config_for("example-01", ""), &fixture.provider,
                                      &fixture.adapter);
  ...
  if (report("bind_port", engine.bind_port(binding)) != 0) {
    return 1;
  }
  ...
  if (report("claim_ownership", engine.claim_ownership(claim)) != 0) {
    return 1;
  }
  ...
  configure.envelope.authority.ownership_generation = ownership->generation;
  ...
  std::printf("generation=%s\n", committed->generation.to_string().c_str());
  std::printf("speed=%s\n", committed->speed.rate.to_string().c_str());
