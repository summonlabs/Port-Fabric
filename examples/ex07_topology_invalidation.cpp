// Example 7: topology invalidation. A configuration bound to a structural
// generation must not silently survive an incompatible advance.
#include <cstdio>
#include <string>
...
int main() {
  ...
  std::printf("topology_generation         %s\n",
              engine.set_topology_generation(portfabric::TopologyGeneration::from_value(10))
                  .to_string()
                  .c_str());
  ...
  std::printf("advance_topology            %s\n",
              engine.set_topology_generation(portfabric::TopologyGeneration::from_value(11))
                  .to_string()
                  .c_str());

  portfabric::ReconfigureRequest stale;
  stale.envelope = envelope_for(port, "agent-a", "boot-a", "stale-topology", engine.epoch());
  stale.envelope.authority.ownership = ownership->id;
  stale.envelope.authority.expected_configuration = generation;
  stale.envelope.authority.expected_topology = portfabric::TopologyGeneration::from_value(10);
  stale.configuration = *engine.configuration(port);
  stale.configuration.mtu = *portfabric::Mtu::create(9000);
  std::printf("stale_topology_mutation     %s\n",
              engine.reconfigure(stale).outcome.to_string().c_str());

  portfabric::ReconcileRequest reconcile;
  reconcile.envelope = envelope_for(port, "agent-a", "boot-a", "reconcile-topology", engine.epoch());
  reconcile.envelope.authority.ownership = ownership->id;
  reconcile.envelope.authority.expected_configuration = generation;
  reconcile.detail = "bind current structural context";
  ...
  std::printf("configuration_topology=%s\n",
              engine.configuration(port)->topology_generation.to_string().c_str());
  return 0;
}
